...
/**
 * This is an UnoCSS adaptation of KevinGrajeda/tailwindcss-spring.
 * The original project is MIT licensed, and so is this fork.
 */
...
const generateEase = (bounce: number) => {
 ...
 */
const calculateSettlingDuration = (solver: SpringSolver, perceptualDuration: number) => {
  const step = 1 / 8
...
const createSpringSolver = ({ mass, stiffness, damping, velocity }: SpringConfig): SpringSolver => {
  const w0 = Math.sqrt(stiffness / mass)
  const zeta = damping / (2 * Math.sqrt(stiffness * mass))
  const wd = zeta < 1 ? w0 * Math.sqrt(1 - zeta * zeta) : 0
  const b = zeta < 1 ? (zeta * w0 - velocity) / wd : -velocity + w0

  const solver: SpringSolver = (t) => {
    let displacement
...
const generateSpringValues = (springSolver: SpringSolver, settlingDuration: number) => {
...
const normalizeTime = (points: Point[], settlingDuration: number): Point[] =>
  points.map(([time, value]) =>
    [time / settlingDuration, value])


const generateLinearSyntax = (points: Point[], round: number): string => {
  ...
  const groupedValues: Point[][] = [[points[0]]]
...
const simplifyDouglasPeucker = (points: Point[], tolerance: number): Point[] => {
...
export const presetSpringTransition = () => definePreset({
  ...
  rules: [
    [
      ...
      ([, value]) => {
        ...
        const { durationMultiplier, ease } = generateEase(bounce)
        return {
          'transition-timing-function': ease,
          '--un-spring-duration-multiplier': `${durationMultiplier}`,
        }
      },
      {
        layer: 'overrides',
        autocomplete: [
          'spring-bounce-<num>',
          'spring-bounce-(0|10|20|30|40|50|60|70|80)'
        ]
      }
    ],
    [
      /^spring-duration-(\d+)$/,
      ([, value]) => ({
        'transition-duration': `calc(var(--un-spring-duration-multiplier, 1) * ${value}ms)`,
      }),
      {
        layer: 'overrides',
        autocomplete: [
          'spring-duration-<num>',
          'spring-duration-(75|100|150|200|300|500|700|1000)'
        ]
      }
    ]
  ],
